...
namespace ZhiHeng.Tickets.Wx.App.Service
{
    ...
    {
        private static QrTicket Create(object obj)
        {
            var url = string.Format("https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token={0}", WXService.AccessToken);
        ...
        public static QrTicket CreateTemp(int expireSeconds, int sceneId)
        {
        ...
        /// <param name="accessToken">accessToken</param>
        /// <returns>ticket实体，ticket可以换取二维码，也可以根据url自行生成。</returns>
        public static QrTicket CreateByID(int sceneId)
        {
        ...
        /// <param name="accessToken">accessToken</param>
        /// <returns>ticket实体，ticket可以换取二维码，也可以根据url自行生成。</returns>
        public static QrTicket CreateByStr(string sceneStr)
        {
