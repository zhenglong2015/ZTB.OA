...
namespace WeChatApi.Service
{
    ...
    public class MaterialService
    {
        ...
        public static UpLoadInfo Add(string filePath, MaterialType mediaType, bool IsTemp = true, string videotitle = "", string videointroduction = "")
        {
            try
            {
                //临时素材接口
                ...
                return Utils.PostResult<UpLoadInfo>(formlist, string.Format(url, WXService.AccessToken, mediaType.ToString()));
            }
            catch (Exception e)
            {
        ...
        /// <returns></returns>
        public static UpLoadInfo AddArticle(List<Article> articles)
        {
            var url = string.Format("https://api.weixin.qq.com/cgi-bin/material/add_news?access_token={0}", WXService.AccessToken);
        ...
        /// <returns>url</returns>
        public static string GetTempUrl(string mediaId)
        {
            var url = "http://api.weixin.qq.com/cgi-bin/media/get?access_token={0}&media_id={1}";
            return string.Format(url, mediaId, WXService.AccessToken);
        }
        ...
        public static MaterialCount GetCount()
        {
            var url = string.Format("https://api.weixin.qq.com/cgi-bin/material/get_materialcount?access_token={0}", WXService.AccessToken);
        ...
        /// <param name="accessToken">全局票据</param>
        public static MaterialList GetList(MaterialType mediaType, int index, int count)
        {
            if (count < 1 || count > 20)
            {
            ...
            var url = string.Format("https://api.weixin.qq.com/cgi-bin/material/batchget_material?access_token={0}", WXService.AccessToken);
        ...
        public static MaterialNews Get(string mediaId, out FileStreamInfo stream)
        {
            var url = string.Format("https://api.weixin.qq.com/cgi-bin/material/get_material?access_token={0}", WXService.AccessToken);
        ...
        /// <returns></returns>
        public static ErrorEntity Del(string mediaId)
        {
            var url = string.Format("https://api.weixin.qq.com/cgi-bin/material/del_material?access_token={0}", WXService.AccessToken);
        ...
        /// <returns></returns>
        public static ErrorEntity Update(string mediaId, int index, Article article)
        {
            var url = string.Format("https://api.weixin.qq.com/cgi-bin/material/update_news?access_token={0}", WXService.AccessToken);
            var obj = new
            {
                media_id = mediaId,
                index = index,
        ...
        /// <returns></returns>
        public static string DownImg(string url, string time, int index)
        {
            string menuPath = System.AppDomain.CurrentDomain.BaseDirectory + @"WxImgs\";
            if (!Directory.Exists(menuPath))
                Directory.CreateDirectory(menuPath);
            WebClient mywebclient = new WebClient();
            string newfilename = time + index + "." + url.Substring(url.LastIndexOf('=') + 1);
            string filepath = menuPath + newfilename;
            if (!File.Exists(filepath))
                mywebclient.DownloadFile(url, filepath);
            return @"..\..\WxImgs\" + newfilename;
